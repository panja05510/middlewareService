...
//            Thread.sleep(100000);
	...
	/*******************************************************************************************
	 * Recursive method to iterate through copybook tree and extract metadata for
	 * cobol variables each symbol leaf value will be stored in hashmap all hashmaps
	 * will go into arraylist
	 * 
	 * @throws InterruptedException
	 *****************************************/

	public ArrayList<HashMap<String, String>> getIntermediateList(IItemDetails items,
			ArrayList<HashMap<String, String>> fields, HashMap<String, Integer> allNames) throws InterruptedException {
		if (items == null) {
			return null;
		}
...
//        Thread.sleep(10000);
	...
	public HashMap<String, String> getRecordHashMap(IItemDetails item, String fieldName) {
		...
		String fieldValue = calculateFieldValue(fieldName, item.getValue(), item.getDisplayLength(),
				item.getStorageLength(), item.getType(), item.getPicture());
	...
	public String calculateFieldValue(String fieldName, String fieldValue, int displayLength, int storageLength,
			int typeId, String pic) {
